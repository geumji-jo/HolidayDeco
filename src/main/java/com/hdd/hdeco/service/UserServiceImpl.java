...
public class UserServiceImpl implements UserService {
	...
	public Map<String, Object> verifyEmail(String email) {
		...
		map.put("enableEmail", userMapper.selectUserByEmail(email) == null
				&& userMapper.selectSleepUserByEmail(email) == null && userMapper.selectOutUserByEmail(email) == null);
		return map;
	}
	...
	public void restore(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (insertResult == 1 && deleteResult == 1) {
				...
				out.println("location.href='/';"); // 로그인이 필요하지만 로그인 페이지로 보내지 말자!(여기서 로그인 페이지로 이동하게 되면 로그인 후 referer에 의해서 다시
																						// restore 과정으로 돌아온다.)
			} else {
	...
	public String getNaverLoginApiURL(HttpServletRequest request) {
		...
		try {
			...
			naverApiURL = "https://nid.naver.com/oauth2.0/authorize?response_type=code";
	...
	public String getKakaoLoginApiURL(HttpServletRequest request) {
		String kakaoApiURL = null;

		try {

			String redirectURI = URLEncoder.encode(kakaoRedirectURL, "UTF-8");
			SecureRandom secureRandom = new SecureRandom();
			String state = new BigInteger(130, secureRandom).toString();
	...
	public String getKakaoLoginToken(HttpServletRequest request) {
		...
		String redirectURI = null;
		try {
			redirectURI = URLEncoder.encode(kakaoRedirectURL, "UTF-8");
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		...
		try {
			...
			kakaoApiURL += "client_id=" + kakaoClientId;
			kakaoApiURL += "&client_secret=" + kakaoClientSecret;
			kakaoApiURL += "&redirect_uri=" + redirectURI;
	...
	public UserDTO getKakaoLoginProfile(String accessToken) {
		...
		try {
			...
			JSONObject kakao_account = new JSONObject(sb.toString()).getJSONObject("kakao_account");
			long kakaouniqueId = new JSONObject(sb.toString()).getLong("id");

			System.out.println("-------------------------idValue1" + sb.toString());
			String kakaoId = Long.toString(kakaouniqueId);
	...
	public void kakaoLogin(HttpServletRequest request, HttpServletResponse response, UserDTO kakaoUserDTO) {
		...
		// 로그인 처리
		HttpSession session = request.getSession();
		String loginName = kakaoUserDTO.getName();
		int adminChk = kakaoUserDTO.getAdminCheck();
	...
	public void kakaoJoin(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			out.println("<script>");
			if (naverJoinResult == 1) {
				out.println("alert('카카오 간편가입이 완료되었습니다. 초기 비밀번호는 생년월일 8자리 숫자(예시: 20201217) 입니다.');");
				out.println("location.href='/';");
			} else {
	 ...
	 * accessToken); String reqURL = "https://kapi.kakao.com/v1/user/logout"; try {
	...
	public String getKakaoLogoutApiURL(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			String redirectURI = URLEncoder.encode(kakaoLogoutRedirectURL, "UTF-8");
